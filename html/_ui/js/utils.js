...
CNJS.UTILS.scrollEndEvent = function () {
	var timer = false;
	CNJS.Config.$window.on('scroll', function(e){
		clearTimeout(timer);
...
CNJS.UTILS.isElemInView = function ($el, topOffset, botOffset) {
	...
	var viewTop = CNJS.Config.$window.scrollTop() + topOffset;
	var viewBot = viewTop + CNJS.Config.$window.height() - topOffset;
...
/**
*	Enforces maxlength in textarea (only needed for IE7/8)
**/
CNJS.UTILS.TextareaMaxlength = function ($textarea) {
	var self = this;
	var elTextarea = $textarea;
	var maxlength = parseInt(elTextarea.attr('maxlength'), 10);
	elTextarea.bind('keyup', function (e) {
		if (elTextarea.val().length >= maxlength) {
			elTextarea.val(elTextarea.val().substring(0, maxlength));
		}
	});
};
